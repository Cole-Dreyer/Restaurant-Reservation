...
async function validSeating(req, res, next) {
  ...
  if (partyPeople > tableCapacity) {
    errorsArray.push(
      `That table's capacity won't be able to seat all ${partyPeople} people.`
    );
  }
  // Check if there is a reservation already associated with the table
  if (tableReservationId) {
    errorsArray.push(
      `That table is already occupied by reservation # ${tableReservationId}`
    );
  }
  ...
  return next({
    status: 400,
    message: `There are one or more issues: ${errorsArray.join(", ")}`,
  });
}
